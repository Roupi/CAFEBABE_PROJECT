...
/**
 * RETURN is the representation of the meta instruction that relate to the act of passing a value from a called method to a calling method,
 * this meta instruction include the following bytecode instructions
 * {return,iretrun,dreturn,lreturn,sreturn,areturn}
 ...
 */
public class RETURN extends InstructionGen{
    /**
     * ID used for the logger
     */
    private static final String id = "com.cafebabe.core.instructions.RETURN.";

    /**
     * Constructor of the RETURN representation
     ...
     */
    public RETURN(InstructionHandle ih, short opcode, short length, int numberOfRegisters,int maxInsPos, int insNumber, String methName, String classIndex, String methArgs) {
        super(ih, opcode, length, numberOfRegisters,maxInsPos, insNumber, methName, classIndex,methArgs);

        if (ih.toString().matches(REGEX_RETURN)) {
            nreads = 1;
            nWrites = 0;
        }else{
            nreads = 0;
        ...
        for(int i = 0; i<this.numberOfRegisters; i++){
            CONSTRAINTS.add(IV+i+" = "+NV+i);
        }

        if (ih.toString().matches(REGEX_RETURN)) {
            CONSTRAINTS.add(RI+"1 = "+SP);
            CONSTRAINTS.add("R = "+RV+"1");
        }
        CONSTRAINTS.add(IHS+" = "+OHS);
        CONSTRAINTS.add("unify("+HI+","+HO+")");
